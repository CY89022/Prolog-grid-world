...
% DFS
dfs(Node, Goal, Visited, [Node]) :- Node = Goal.
dfs(Node, Goal, Visited, [Node|Path]) :-
    e(X1,Y1,X2,Y2,_),
    (Node = (X1,Y1), Next = (X2,Y2);
     Node = (X2,Y2), Next = (X1,Y1)),
    \+ member(Next, Visited),
    dfs(Next, Goal, [Next|Visited], Path).

% DLS
dls(Node, Goal, Limit, Path) :-
    dls_helper(Node, Goal, [Node], Limit, Path).

dls_helper(Node, Goal, _, 0, []) :- Node \= Goal, !, fail.
dls_helper(Node, Goal, Visited, _, [Node]) :- Node = Goal.
dls_helper(Node, Goal, Visited, Limit, [Node|Path]) :-
    Limit > 0,
    e(X1,Y1,X2,Y2,_),
    (Node = (X1,Y1), Next = (X2,Y2);
     Node = (X2,Y2), Next = (X1,Y1)),
    \+ member(Next, Visited),
    NewLimit is Limit - 1,
    dls_helper(Next, Goal, [Next|Visited], NewLimit, Path).
...
run_dfs_tri(Start, Goal, MaxX, MaxY) :-
    gvi,
...
run_dls_tri(Start, Goal, MaxX, MaxY, Limit) :-
    gvi,
    gen_tri_grid(MaxX, MaxY),
    connect_odd_row(MaxX, MaxY),
    dls(Start, Goal, Limit, Path),
...
run_ids_tri(Start, Goal, MaxX, MaxY) :-
    gvi,
    gen_tri_grid(MaxX, MaxY),
    connect_odd_row(MaxX, MaxY),
    ids(Start, Goal, Path),
...
run_dfs_rect(Start, Goal, MaxX, MaxY) :-
    dfs(Start, Goal, [Start], Path),
    gvi,
    draw_edges_rect,
...
run_dls_rect(Start, Goal, Limit, MaxX, MaxY) :-
    dls(Start, Goal, Limit, Path),
    gvi,
    draw_edges_rect,
    label_nodes_rect(MaxX, MaxY),
...
run_ids_rect(Start, Goal, MaxX, MaxY) :-
    ids(Start, Goal, Path),
    gvi,
    draw_edges_rect,
    label_nodes_rect(MaxX, MaxY),
...
run_dfs_hex(Start, Goal) :-
    dfs(Start, Goal, [Start], Path),
    gvi, draw_edges_hex, label_nodes, draw_hex_path(Path), gvf.

run_dls_hex(Start, Goal, Limit) :-
    dls(Start, Goal, Limit, Path),
    gvi, draw_edges_hex, label_nodes, draw_hex_path(Path), gvf.

run_ids_hex(Start, Goal) :-
    ids(Start, Goal, Path),
    gvi, draw_edges_hex, label_nodes, draw_hex_path(Path), gvf.
